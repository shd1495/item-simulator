...
## 필수 기능
 - [x] 회원가입 API - /sign_up
 - [x] 로그인 API (JWT 토큰 발급) - /sign_in
 - [x] 캐릭터 생성 API (JWT 인증만 가능) - /char
 - [x] 캐릭터 삭제 API (JWT 인증만 가능) - /char/:char_id
 - [x] 캐릭터 상세 조회 API (JWT 부분 인증) - /char/:char_id
 - [x] 아이템 생성 API - /items
 - [x] 아이템 수정 API - /items/:item_code
 - [x] 아이템 목록 조회 API - /items
 - [x] 아이템 상세 조회 API - /items/:item_code

## 도전 기능
 - [x] 아이템 구입 API (JWT 인증만 가능) - /buy/:char_id
 - [x] 아이템 판매 API (JWT 인증만 가능) - /sell/:char_id
 - [x] 캐릭터가 보유한 인벤토리 내 아이템 목록 조회 API (JWT 인증만 가능) - /char/inventory/:char_id
 - [x] 캐릭터가 장착한 아이템 목록 조회 API - /char/equip/:char_id
 - [x] 아이템 장착 API (JWT 인증만 가능) - /equip/:char_id
 - [x] 아이템 장착 해제 API (JWT 인증만 가능) - /unEquip/:char_id
 - [x] 게임 머니 획득 API (JWT 인증만 가능) - /char/:char_id
...
1. Auth 헤더로 토큰을 넘긴다는 개념이 잘 이해가 안되서 많이 찾아봤습니다.
  
   jwt를 Auth 헤더로 전달하는 방식으로 인가를 할 경우 세션스토리지 등 서버에서 jwt 토큰을 관리할 필요가 없기 때문에
 
   관련 라이브러리를 삭제하는 등의 해프닝이 있었습니다.
---
2. 커밋 컨벤션을 정하고 과제를 진행했는데 평소 github desktop 같은 gui로만 커밋 푸쉬 등을 했었기 때문에
...
---
3. AWS EC2 인스턴스를 생성할 때 문제가 생겼습니다. 서울 지역 동북아b 가용 영역에서는 t2.micro를 사용할 수 없는 문제가 있어서 이유를 찾아보았더니
